import random
import asyncio
import logging
from aiogram import Bot, Dispatcher
from apscheduler.schedulers.asyncio import AsyncIOScheduler
from apscheduler.events import EVENT_JOB_ERROR

# Настройка логирования
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

# Токен бота и ID группы
TOKEN = "8086402680:AAHXsLs0eZNuNOS8P4Hx9DpY_kOd_IZ26yc"
GROUP_CHAT_ID = -1002338597070

# Инициализация бота, диспетчера и планировщика задач
bot = Bot(token=TOKEN)
dp = Dispatcher()
scheduler = AsyncIOScheduler()

# Список заданий
weekly_tasks = [
    "Сегодня задача простая: похвали кого-то из команды. Не в голове. Вслух или текстом.",
    "Поймай момент тишины между задачами и подумай: что сегодня получилось хорошо?",
    "Сделай рабочее место чуть уютнее. Один маленький штрих.",
    "Отправь коллеге мем, который идеально описывает эту неделю.",
    "На 10 минут выйди из чатов. Мир не рухнет, проверено волками.",
    "Спроси у коллеги, как он на самом деле. И выслушай.",
    "Вспомни, зачем ты вообще этим занимаешься. Ответ можешь оставить при себе.",
]

# Задача для вторника
async def send_tuesday_task():
    task = random.choice(weekly_tasks)
    text = (
        "🐺 Волк на связи.\n\n"
        "Вторник — значит время маленькой задачи:\n\n"
        f"👉 {task}\n\n"
        "Без отчётов. Просто попробуй."
    )
    try:
        await bot.send_message(GROUP_CHAT_ID, text)
    except Exception as e:
        logger.error(f"Ошибка при отправке сообщения в группу: {e}")

# Задача для пятницы (Мак)
async def send_friday_mac():
    text = (
        "🐺 Пятница. Утро.\n\n"
        "Волк напоминает:\n"
        "после работы надо заехать в мак 🍔\n\n"
        "Работа подождёт. Картошка — нет."
    )
    try:
        await bot.send_message(GROUP_CHAT_ID, text)
    except Exception as e:
        logger.error(f"Ошибка при отправке сообщения в группу: {e}")

# Планировщик задач
scheduler.add_job(send_tuesday_task, "interval", minutes=1)
scheduler.add_job(send_thursday_task, "interval", minutes=1)
scheduler.add_job(send_tuesday_task, "cron", day_of_week="tue", hour=11, minute=0)
scheduler.add_job(send_friday_mac, "cron", day_of_week="fri", hour=10, minute=0)

# Проверка работы - Чтобы убедиться, что всё работает корректно, можешь временно заменить cron-расписание на более короткий интервал: scheduler.add_job(send_tuesday_task, "interval", minutes=1) scheduler.add_job(send_thursday_task, "interval", minutes=1)
# Проверь, отправляется ли сообщение. Если всё работает — верни обратно cron: scheduler.add_job(send_tuesday_task, "cron", day_of_week="tue", hour=11, minute=0)

# Обработчик ошибок планировщика
def job_error_listener(event):
    if event.exception:
        logger.error(f"Ошибка при выполнении задачи: {event.job_id}")

scheduler.add_listener(job_error_listener, EVENT_JOB_ERROR)

# Основная асинхронная функция для запуска бота
async def main():
    scheduler.start()  # Запуск планировщика
    await dp.start_polling(bot)  # Запуск бота

# Запуск бота
if __name__ == "__main__":
    asyncio.run(main())

